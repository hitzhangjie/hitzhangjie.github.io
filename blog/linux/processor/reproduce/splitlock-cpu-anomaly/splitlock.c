...
static void check_split_lock_detection(void) {
    FILE *f = fopen("/proc/sys/kernel/split_lock_mitigation", "r");
    if (f) {
        char buf[32] = {0};
        if (fgets(buf, sizeof(buf), f)) {
            printf("[splitlock] kernel split_lock_mitigation = %s", buf);
            if (buf[0] != '0') {
                printf("[splitlock] WARNING: split-lock detection is enabled.\n"
                       "[splitlock]   Detection may SIGBUS this process. To disable:\n"
                       "[splitlock]   # sysctl -w kernel.split_lock_mitigation=0\n");
            }
        }
        fclose(f);
    } else {
        printf("[splitlock] /proc/sys/kernel/split_lock_mitigation not available "
               "(kernel < 5.7 or not x86)\n");
    }
