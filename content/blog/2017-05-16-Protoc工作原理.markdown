...
### 2.2. protoc命令行接口定义
下面看一下protoc的命令行接口定义，以了解其对命令行flags、options的解释过程以及对后续程序执行逻辑的影响。
...
#### 2.3.1. protoc完成基本的初始化工作后调用cli.Run(argc,argv)开始生成代码
...
##### 2.3.3.1 protoc CommandLineInterface::GenerateOutput(...)实现
