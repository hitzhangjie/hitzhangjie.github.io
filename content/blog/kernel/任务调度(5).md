...
而它也可以建模多层级结构下的调度的公平性，如`不同用户->不同会话->不同任务组->不同线程`的各级调度均保证公平。实际上在Linux 2.6.24还是Linux 2.6.30的CFS补丁中确实有一个选项，CONFIG_FAIR_USER_SCHED=y，编译构建时设置为打开，那么CFS调度器会自动对不同用户线进行公平调度。但是后续又移除了这个编译选项，感兴趣的可以继续看下这两篇内容：
...
>ps: 插一个问题，进程数、线程数多了之后，CFS调度器下线程的切换频率是否会升高呢？
>
>会的！直观理解就是：
